...
@Composable
fun EditProfileScreen(userContext: UserContext) {
    var username by remember { mutableStateOf(userContext.userState.username) }
    var email by remember { mutableStateOf(userContext.userState.email) }
    ...
    Column(
        ...
        Button(
            onClick = { userContext.updateProfile(username, email, password) { status = it } },
            modifier = Modifier.fillMaxWidth()
        ) {
            Text("Update")
        }
